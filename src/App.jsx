...
// ─── BRACKET TAB (CDL Double Elimination) ───────────────────

var BRACKET_KEY = "barracks_bracket";
var BRACKET_SCORES = ["3-0", "3-1", "3-2"];
var WR1_SEEDS = [[0,7], [3,4], [1,6], [2,5]];

function loadBracketData() {
  try { var raw = localStorage.getItem(BRACKET_KEY); return raw ? JSON.parse(raw) : null; } catch(e) { return null; }
}
function saveBracketData(data) {
  try { localStorage.setItem(BRACKET_KEY, JSON.stringify(data)); } catch(e) {}
}

function encodeBracketParam(data) {
  var seedStr = (data.seeds || []).map(function(s) { return s ? s.id : "_"; }).join(",");
  var pickParts = [];
  var picks = data.picks || {};
  Object.keys(picks).forEach(function(mid) {
    var p = picks[mid];
    if (p && p.winner != null && p.score) pickParts.push(mid + "." + p.winner + "." + p.score);
  });
  return seedStr + "|" + pickParts.join(",");
}

function decodeBracketParam(param, cdlTeams) {
  var parts = param.split("|");
  var result = {seeds: [], picks: {}};
  if (parts[0]) {
    result.seeds = parts[0].split(",").map(function(id) {
      if (id === "_") return null;
      return cdlTeams.find(function(t) { return t.id === Number(id); }) || null;
    });
  }
  if (parts[1]) {
    parts[1].split(",").forEach(function(chunk) {
      var segs = chunk.split(".");
      if (segs.length === 3) result.picks[segs[0]] = {winner: Number(segs[1]), score: segs[2]};
    });
  }
  return result;
}

function computeBracketSlots(seeds, picks) {
  var sd = function(i) { return seeds && seeds[i] ? seeds[i] : null; };
  var pw = function(mid) { return picks[mid] && picks[mid].winner != null ? picks[mid].winner : null; };
  var gt = function(id) {
    if (id == null) return null;
    for (var i = 0; i < (seeds || []).length; i++) { if (seeds[i] && seeds[i].id === id) return seeds[i]; }
    return null;
  };
  var slots = {};

  slots.w1 = {t1: sd(WR1_SEEDS[0][0]), t2: sd(WR1_SEEDS[0][1])};
  slots.w2 = {t1: sd(WR1_SEEDS[1][0]), t2: sd(WR1_SEEDS[1][1])};
  slots.w3 = {t1: sd(WR1_SEEDS[2][0]), t2: sd(WR1_SEEDS[2][1])};
  slots.w4 = {t1: sd(WR1_SEEDS[3][0]), t2: sd(WR1_SEEDS[3][1])};

  slots.w5 = {t1: gt(pw("w1")), t2: gt(pw("w2"))};
  slots.w6 = {t1: gt(pw("w3")), t2: gt(pw("w4"))};
  slots.wf = {t1: gt(pw("w5")), t2: gt(pw("w6"))};

  var loser = function(mid) {
    var p = picks[mid]; if (!p || p.winner == null) return null;
    var sl = slots[mid]; if (!sl) return null;
    if (sl.t1 && sl.t1.id === p.winner) return sl.t2;
    if (sl.t2 && sl.t2.id === p.winner) return sl.t1;
    return null;
  };

  slots.l1 = {t1: loser("w1"), t2: loser("w4")};
  slots.l2 = {t1: loser("w2"), t2: loser("w3")};
  slots.l3 = {t1: gt(pw("l1")), t2: loser("w5")};
  slots.l4 = {t1: gt(pw("l2")), t2: loser("w6")};
  slots.l5 = {t1: gt(pw("l3")), t2: gt(pw("l4"))};
  slots.lf = {t1: gt(pw("l5")), t2: loser("wf")};
  slots.gf = {t1: gt(pw("wf")), t2: gt(pw("lf"))};

  return slots;
}

var DOWNSTREAM = {
  w1: ["w5","l1","l3","l5","lf","gf","wf"], w2: ["w5","l2","l3","l4","l5","lf","gf","wf"],
  w3: ["w6","l2","l4","l5","lf","gf","wf"], w4: ["w6","l1","l3","l4","l5","lf","gf","wf"],
  w5: ["wf","l3","l5","lf","gf"], w6: ["wf","l4","l5","lf","gf"],
  wf: ["lf","gf"], l1: ["l3","l5","lf","gf"], l2: ["l4","l5","lf","gf"],
  l3: ["l5","lf","gf"], l4: ["l5","lf","gf"], l5: ["lf","gf"], lf: ["gf"], gf: []
};

function BracketMatch(props) {
  var matchId = props.matchId, slot = props.slot, pick = props.pick, onPick = props.onPick, roundLabel = props.roundLabel;
  var t1 = slot.t1, t2 = slot.t2;
  var ready = t1 && t2;
  var winnerId = pick ? pick.winner : null;
  var score = pick ? pick.score : null;
  ...
  var handleTeamClick = function(team) {
    ...
    if (winnerId === team.id) {
      ...
      setPendingWinner(team.id);
      setShowScores(true);
    }
  };

  var handleScoreClick = function(sc) {
    if (!pendingWinner) return;
    onPick(matchId, {winner: pendingWinner, score: sc});
    setShowScores(false);
    setPendingWinner(null);
  };

  // Reset score picker if pick is cleared externally
  useEffect(function() {
    if (!pick) { setShowScores(false); setPendingWinner(null); }
  }, [pick]);

  var teamSlot = function(team, isWinner, side) {
    if (!team) return <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)", minHeight: "40px"}}>
      <div className="w-1 h-5 rounded-full" style={{background: "#333"}} />
      <span className="text-xs" style={{color: "#333"}}>TBD</span>
    </div>;

    var isPending = pendingWinner === team.id && !winnerId;
    return <button onClick={function() { handleTeamClick(team); }} className="flex items-center gap-2 px-3 py-2 rounded-lg w-full transition-all" style={{
      background: isWinner ? "rgba(82,183,136,0.1)" : isPending ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.03)",
      border: isWinner ? "2px solid rgba(82,183,136,0.3)" : isPending ? "2px solid rgba(255,255,255,0.2)" : "2px solid rgba(255,255,255,0.06)",
      cursor: ready ? "pointer" : "default",
      minHeight: "40px"
    }}>
      <div className="w-1 h-5 rounded-full flex-shrink-0" style={{background: team.color || "#888"}} />
      <span className="text-sm font-bold text-white flex-1 text-left truncate">{team.abbr || team.name || "?"}</span>
      {isWinner && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#52b788" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0"><polyline points="20 6 9 17 4 12"/></svg>}
    </button>;
  };

  var hasPick = winnerId && score;

  return <div className="rounded-xl overflow-hidden" style={{
    background: hasPick ? "rgba(82,183,136,0.04)" : "rgba(255,255,255,0.03)",
    border: hasPick ? "1px solid rgba(82,183,136,0.12)" : "1px solid rgba(255,255,255,0.06)",
    transition: "all 0.2s"
  }}>
    {/* Round label */}
    <div className="px-3 pt-2 pb-1 flex items-center justify-between">
      <span style={{fontSize: "9px", color: "#555", textTransform: "uppercase", letterSpacing: "0.5px"}}>{roundLabel}</span>
      {hasPick && <button onClick={function() { onPick(matchId, null); }} className="text-xs px-1.5 py-0.5 rounded opacity-40 hover:opacity-80" style={{color: "#888", fontSize: "9px"}}>clear</button>}
    </div>

    {/* Teams */}
    <div className="px-3 pb-2 space-y-1.5">
      {teamSlot(t1, winnerId && t1 && winnerId === t1.id, "top")}
      {teamSlot(t2, winnerId && t2 && winnerId === t2.id, "bot")}
    </div>

    {/* Score picker */}
    {showScores && pendingWinner && <div className="px-3 pb-3 pt-1" style={{borderTop: "1px solid rgba(255,255,255,0.04)"}}>
      <div className="flex items-center gap-2">
        <span style={{fontSize: "10px", color: "#555"}}>Score:</span>
        <div className="flex gap-1.5">
          {BRACKET_SCORES.map(function(sc) {
            return <button key={sc} onClick={function() { handleScoreClick(sc); }} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all" style={{
              background: "rgba(82,183,136,0.1)", color: "#52b788", border: "1px solid rgba(82,183,136,0.2)"
            }}>{sc}</button>;
          })}
        </div>
      </div>
    ...
    {/* Pick summary */}
    {hasPick && <div className="px-3 pb-2">
      <div className="flex items-center gap-1.5">
        <div className="w-1 h-3 rounded-full" style={{background: (winnerId === (t1 && t1.id) ? (t1 && t1.color) : (t2 && t2.color)) || "#888"}} />
        <span className="text-xs font-bold" style={{color: "#52b788"}}>
          {winnerId === (t1 && t1.id) ? (t1 && t1.abbr) : (t2 && t2.abbr)} wins {score}
        </span>
      </div>
    </div>}
  </div>;
...
function SeedSelector(props) {
  var cdlTeams = props.cdlTeams, seeds = props.seeds, onSeedsChange = props.onSeedsChange;
  var [available, setAvailable] = useState(function() {
    var usedIds = {};
    (seeds || []).forEach(function(s) { if (s) usedIds[s.id] = true; });
    return cdlTeams.filter(function(t) { return !usedIds[t.id]; });
  });

  var handleAdd = function(team, slotIndex) {
    var newSeeds = (seeds || []).slice();
    while (newSeeds.length <= slotIndex) newSeeds.push(null);
    newSeeds[slotIndex] = team;
    onSeedsChange(newSeeds);
    setAvailable(function(prev) { return prev.filter(function(t) { return t.id !== team.id; }); });
  };

  var handleRemove = function(slotIndex) {
    var newSeeds = (seeds || []).slice();
    var removed = newSeeds[slotIndex];
    newSeeds[slotIndex] = null;
    onSeedsChange(newSeeds);
    if (removed) {
  ...
  var handleSwap = function(fromIdx, toIdx) {
    var newSeeds = (seeds || []).slice();
    var temp = newSeeds[fromIdx];
    newSeeds[fromIdx] = newSeeds[toIdx];
    newSeeds[toIdx] = temp;
    onSeedsChange(newSeeds);
  };

  var seedSlots = [];
  for (var i = 0; i < 8; i++) seedSlots.push(i);

  return <div>
    <div className="mb-4">
      <div className="text-xs uppercase tracking-wider mb-3" style={{color: "#555"}}>Seeds (1-8)</div>
      <div className="space-y-2">
        {seedSlots.map(function(idx) {
          var team = seeds && seeds[idx] ? seeds[idx] : null;
          return <div key={idx} className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0" style={{
              background: idx < 2 ? "rgba(233,69,96,0.2)" : idx < 4 ? "rgba(83,168,182,0.15)" : "rgba(255,255,255,0.06)",
              color: idx < 2 ? "#e94560" : idx < 4 ? "#53a8b6" : "#666"
            }}>{idx + 1}</div>
            {team ? <div className="flex items-center gap-2 flex-1 px-3 py-2 rounded-lg" style={{background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)"}}>
              <div className="w-1 h-5 rounded-full" style={{background: team.color || "#888"}} />
              <span className="text-sm font-bold text-white flex-1">{team.name}</span>
              <span className="text-xs opacity-40">{team.abbr}</span>
              {/* Move buttons */}
              {idx > 0 && <button onClick={function() { handleSwap(idx, idx - 1); }} className="text-xs px-1 opacity-30 hover:opacity-80" title="Move up">{"\u25B2"}</button>}
              {idx < 7 && seeds && seeds[idx + 1] && <button onClick={function() { handleSwap(idx, idx + 1); }} className="text-xs px-1 opacity-30 hover:opacity-80" title="Move down">{"\u25BC"}</button>}
              <button onClick={function() { handleRemove(idx); }} className="text-xs px-1.5 py-0.5 rounded opacity-40 hover:opacity-80" style={{color: "#ff6b6b"}}>{"\u2715"}</button>
            </div> : <div className="flex-1 px-3 py-2 rounded-lg" style={{background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.06)", minHeight: "36px"}}>
              <span className="text-xs" style={{color: "#333"}}>Select a team...</span>
            </div>}
          </div>;
        })}
      </div>
    ...
    {available.length > 0 && <div>
      <div className="text-xs uppercase tracking-wider mb-2" style={{color: "#555"}}>Available teams</div>
      <div className="flex flex-wrap gap-2">
        {available.map(function(team) {
          // Find first empty slot
          var emptyIdx = -1;
          for (var i = 0; i < 8; i++) { if (!seeds || !seeds[i]) { emptyIdx = i; break; } }
          if (emptyIdx === -1) return null;
          var capturedIdx = emptyIdx;
          return <button key={team.id} onClick={function() { handleAdd(team, capturedIdx); }} className="flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all hover:bg-white/5" style={{
            background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)"
          }}>
            <div className="w-1 h-4 rounded-full" style={{background: team.color || "#888"}} />
...
function BracketTab(props) {
  var analysis = props.analysis;
  var [phase, setPhase] = useState("seed"); // "seed" or "bracket"
  var [seeds, setSeeds] = useState([]);
  var [picks, setPicks] = useState({});
  var [linkCopied, setLinkCopied] = useState(false);
  var [sharing, setSharing] = useState(false);

  // Get CDL teams for seed selection
  var cdlTeams = useMemo(function() {
    var teams = [];
    var seen = {};
    // From teamStats (has all 12 CDL teams with stats)
    Object.values(analysis.teamStats).forEach(function(ts) {
      if (!seen[ts.team_id]) {
        seen[ts.team_id] = true;
        teams.push({id: ts.team_id, name: ts.team_name, abbr: ts.team_abbr || ts.team_short, color: ts.team_color});
      }
    });
  ...
  // Load saved bracket or shared bracket from URL
  useEffect(function() {
    try {
      var params = new URLSearchParams(window.location.search);
      var shared = params.get("bracket");
      if (shared) {
        var decoded = decodeBracketParam(shared, cdlTeams);
        if (decoded.seeds && decoded.seeds.filter(Boolean).length === 8) {
          setSeeds(decoded.seeds);
    ...
    var saved = loadBracketData();
    if (saved && saved.seeds && saved.seeds.filter(Boolean).length > 0) {
      // Rehydrate team objects from IDs
      var rehydrated = (saved.seedIds || []).map(function(id) {
        return cdlTeams.find(function(t) { return t.id === id; }) || null;
      });
      if (rehydrated.filter(Boolean).length === 8) {
        setSeeds(rehydrated);
  ...
  // Save to localStorage whenever seeds/picks change
  useEffect(function() {
    if (seeds.filter(Boolean).length > 0) {
      saveBracketData({
        seedIds: seeds.map(function(s) { return s ? s.id : null; }),
        picks: picks
      });
    }
  }, [seeds, picks]);

  var handleSeedsChange = function(newSeeds) {
    setSeeds(newSeeds);
    // Clear picks when seeds change since the bracket is different now
    setPicks({});
  };

  var handleStartBracket = function() {
    if (seeds.filter(Boolean).length === 8) setPhase("bracket");
  };

  var handleBackToSeeds = function() {
    setPhase("seed");
  };
  ...
  var handlePick = function(matchId, pick) {
    setPicks(function(prev) {
      ...
      if (!pick) {
        ...
        (DOWNSTREAM[matchId] || []).forEach(function(dm) { delete next[dm]; });
      } else {
        // If changing winner, clear downstream
        if (prev[matchId] && prev[matchId].winner !== pick.winner) {
          (DOWNSTREAM[matchId] || []).forEach(function(dm) { delete next[dm]; });
        }
        next[matchId] = pick;
      }
  ...
  var handleClearBracket = function() {
    setPicks({});
  };
  ...
  var handleResetAll = function() {
    ...
    setPicks({});
    setPhase("seed");
    try { localStorage.removeItem(BRACKET_KEY); } catch(e) {}
  };

  var handleCopyLink = function() {
    var data = {seeds: seeds, picks: picks};
    var encoded = encodeBracketParam(data);
    var url = window.location.origin + window.location.pathname + "?bracket=" + encodeURIComponent(encoded);
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url).then(function() {
        setLinkCopied(true); setTimeout(function() { setLinkCopied(false); }, 2000);
      }).catch(function() { prompt("Copy this link:", url); });
  ...
  var handleShareImage = function() {
    ...
    var data = {seeds: seeds, picks: picks};
    var encoded = encodeBracketParam(data);
    var shareUrl = window.location.origin + window.location.pathname + "?bracket=" + encodeURIComponent(encoded);
    import("./shareRenderer.js").then(function(mod) {
      return mod.shareBracketImage(seeds, picks, computeBracketSlots(seeds, picks), shareUrl);
    }).then(function() { setSharing(false); }).catch(function(e) { console.error(e); setSharing(false); });
  };

  var slots = computeBracketSlots(seeds, picks);
  var totalPicks = Object.keys(picks).filter(function(k) { return picks[k] && picks[k].winner != null && picks[k].score; }).length;
  var champion = picks.gf ? (seeds || []).find(function(s) { return s && s.id === picks.gf.winner; }) : null;

  // SEED PHASE
  if (phase === "seed") {
    var seedCount = seeds.filter(Boolean).length;
    return <div>
      <div className="mb-4">
        <h2 className="text-lg font-bold text-white mb-1">Bracket predictor</h2>
        <p className="text-xs" style={{color: "#555"}}>Select 8 teams and seed them to build your bracket</p>
      </div>

      <SeedSelector cdlTeams={cdlTeams} seeds={seeds} onSeedsChange={handleSeedsChange} />

      <div className="mt-4 flex gap-2">
        <button onClick={handleStartBracket} disabled={seedCount < 8} className="flex-1 py-3 rounded-xl text-sm font-bold transition-all" style={{
          background: seedCount === 8 ? "#e94560" : "rgba(255,255,255,0.04)",
          color: seedCount === 8 ? "#fff" : "#555",
          opacity: seedCount === 8 ? 1 : 0.5
        }}>
          {seedCount === 8 ? "Start bracket \u2192" : seedCount + "/8 teams selected"}
        </button>
      </div>

      {/* WR1 Preview */}
      {seedCount === 8 && <div className="mt-4 rounded-xl p-3" style={{background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)"}}>
        <div className="text-xs uppercase tracking-wider mb-2" style={{color: "#555"}}>Winners Round 1 matchups</div>
        {WR1_SEEDS.map(function(pair, i) {
          var a = seeds[pair[0]], b = seeds[pair[1]];
          return <div key={i} className="flex items-center gap-2 py-1.5" style={{borderBottom: i < 3 ? "1px solid rgba(255,255,255,0.03)" : "none"}}>
            <span className="text-xs font-bold" style={{color: "#e94560", width: "16px"}}>#{pair[0] + 1}</span>
            <div className="w-1 h-4 rounded-full" style={{background: a ? a.color : "#333"}} />
            <span className="text-sm font-bold text-white flex-1">{a ? a.abbr : "?"}</span>
            <span className="text-xs" style={{color: "#555"}}>vs</span>
            <span className="text-sm font-bold text-white flex-1 text-right">{b ? b.abbr : "?"}</span>
            <div className="w-1 h-4 rounded-full" style={{background: b ? b.color : "#333"}} />
            <span className="text-xs font-bold" style={{color: "#555", width: "16px"}}>#{pair[1] + 1}</span>
          </div>;
        })}
      </div>}
    </div>;
  ...
  // BRACKET PHASE
  var WINNER_ROUNDS = [
    {label: "Winners Round 1", matches: ["w1", "w2", "w3", "w4"]},
    {label: "Winners Round 2", matches: ["w5", "w6"]},
    {label: "Winners Final", matches: ["wf"]}
  ];
  var LOSER_ROUNDS = [
    {label: "Losers Round 1", matches: ["l1", "l2"]},
    {label: "Losers Round 2", matches: ["l3", "l4"]},
    {label: "Losers Round 3", matches: ["l5"]},
    {label: "Losers Final", matches: ["lf"]}
  ];
  ...
  return <div>
    <div className="mb-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white mb-1">Bracket predictor</h2>
          <p className="text-xs" style={{color: "#555"}}>{totalPicks}/13 matches predicted</p>
        </div>
        <button onClick={handleBackToSeeds} className="text-xs font-semibold" style={{color: "#e94560"}}>{"\u2190"} Edit seeds</button>
      </div>
    </div>

    {/* Champion banner */}
    {champion && <div className="rounded-xl p-4 mb-4 text-center" style={{
      background: "rgba(82,183,136,0.08)", border: "1px solid rgba(82,183,136,0.2)"
    }}>
      <div style={{fontSize: "10px", color: "#52b788", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "4px"}}>Champion</div>
      <div className="flex items-center justify-center gap-2">
        <div className="w-2 h-8 rounded" style={{background: champion.color || "#888"}} />
    ...
    <div className="rounded-xl p-4 mb-4" style={{background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)"}}>
      <div className="flex items-center gap-3 mb-3">
        <div className="flex-1">
          <div className="h-2 rounded-full overflow-hidden" style={{background: "rgba(255,255,255,0.06)"}}>
            <div className="h-full rounded-full transition-all" style={{width: (totalPicks / 13 * 100) + "%", background: totalPicks === 13 ? "#52b788" : "#e94560"}} />
          </div>
        </div>
        <span className="text-xs font-bold" style={{color: totalPicks === 13 ? "#52b788" : "#888"}}>{totalPicks}/13</span>
      </div>
      {totalPicks > 0 && <div className="flex gap-2">
        <button onClick={handleShareImage} disabled={sharing} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold" style={{background: sharing ? "rgba(233,69,96,0.3)" : "#e94560", color: "#fff", opacity: sharing ? 0.7 : 1}}>
          {sharing ? "Generating..." : "Share bracket"}
        </button>
        <button onClick={handleCopyLink} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-bold" style={{background: linkCopied ? "rgba(82,183,136,0.15)" : "rgba(255,255,255,0.06)", color: linkCopied ? "#52b788" : "#888", border: linkCopied ? "1px solid rgba(82,183,136,0.3)" : "1px solid rgba(255,255,255,0.08)"}}>
          {linkCopied ? "Copied!" : "Copy link"}
        </button>
      </div>}
      {totalPicks > 0 && <div className="mt-3 pt-3 flex justify-between" style={{borderTop: "1px solid rgba(255,255,255,0.04)"}}>
        <button onClick={handleClearBracket} className="text-xs px-3 py-1.5 rounded-lg font-semibold" style={{background: "rgba(255,107,107,0.08)", color: "#ff6b6b", border: "1px solid rgba(255,107,107,0.15)", opacity: 0.7}}>Clear picks</button>
        <button onClick={handleResetAll} className="text-xs px-3 py-1.5 rounded-lg font-semibold" style={{background: "rgba(255,255,255,0.04)", color: "#666", border: "1px solid rgba(255,255,255,0.06)"}}>Reset everything</button>
      </div>}
    </div>
    ...
    <div className="mb-6">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-1 h-4 rounded" style={{background: "#52b788"}} />
        <span className="text-sm font-bold text-white uppercase tracking-wider">Winners bracket</span>
      </div>
      {WINNER_ROUNDS.map(function(round) {
        return <div key={round.label} className="mb-4">
          <div className="text-xs font-bold uppercase tracking-wider mb-2" style={{color: "#555"}}>{round.label}</div>
          <div className={"grid gap-3 " + (round.matches.length > 2 ? "grid-cols-1 sm:grid-cols-2" : round.matches.length === 2 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1")}>
            {round.matches.map(function(mid) {
              return <BracketMatch key={mid} matchId={mid} slot={slots[mid]} pick={picks[mid] || null} onPick={handlePick} roundLabel={round.label} />;
            })}
          </div>
        </div>;
      })}
    ...
    <div className="mb-6">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-1 h-4 rounded" style={{background: "#ff6b6b"}} />
        <span className="text-sm font-bold text-white uppercase tracking-wider">Losers bracket</span>
      </div>
      {LOSER_ROUNDS.map(function(round) {
        return <div key={round.label} className="mb-4">
          <div className="text-xs font-bold uppercase tracking-wider mb-2" style={{color: "#555"}}>{round.label}</div>
          <div className={"grid gap-3 " + (round.matches.length >= 2 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1")}>
            {round.matches.map(function(mid) {
              return <BracketMatch key={mid} matchId={mid} slot={slots[mid]} pick={picks[mid] || null} onPick={handlePick} roundLabel={round.label} />;
            })}
          </div>
    ...
    <div className="mb-4">
      <div className="flex items-center gap-2 mb-3">
        <div className="w-1 h-4 rounded" style={{background: "#ffd166"}} />
        <span className="text-sm font-bold text-white uppercase tracking-wider">Grand Final</span>
      </div>
      <BracketMatch matchId="gf" slot={slots.gf} pick={picks.gf || null} onPick={handlePick} roundLabel="Grand Final" />
    </div>
  </div>;
}
...
var TABS = ["Schedule", "Picks", "Bracket", "Rankings", "Teams", "Compare", "Players", "Lines", "Search"];
...
export default function App() {
  ...
  var bracketParam = urlParams.get("bracket");
  var [tab, setTab] = useState(compareParam ? "Compare" : lineParam ? "Lines" : picksParam ? "Picks" : bracketParam ? "Bracket" : "Schedule");
  var [loading, setLoading] = useState(true);
  ...
  return <div className="min-h-screen" style={{background: "#0d0d1a", color: "#c8c8d0"}}>
    ...
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {tab === "Bracket" && <BracketTab analysis={analysis} />}
