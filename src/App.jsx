...
async function fetchPlayers() {
  const p = {"0":{"json":{"eventType":[],"mapId":ALL_MAP_IDS,"modeId":[1,2,3,4,5],"eventId":[],"teamId":[],"onlyCDLStats":true,"onlyChallengersStats":false,"seasonId":2026,"startAt":null,"endAt":null,"aggregateMatchStats":true},"meta":{"values":{"startAt":["undefined"],"endAt":["undefined"]}}}};
  const d = await proxyFetch(`https://www.breakingpoint.gg/api/trpc/playerStats.getAggregatedOrderedPlayerStats?batch=1&input=${encodeURIComponent(JSON.stringify(p))}`);
  return d?.[0]?.result?.data?.json || [];
}
async function fetchTeams() {
  const p = {"0":{"json":{"eventType":[],"mapId":ALL_MAP_IDS,"modeId":[1,2,3,4,5],"eventId":[],"teamId":[],"onlyCDLStats":true,"onlyChallengersStats":false,"seasonId":2026,"startAt":null,"endAt":null},"meta":{"values":{"startAt":["undefined"],"endAt":["undefined"]}}}};
  const d = await proxyFetch(`https://www.breakingpoint.gg/api/trpc/teamStats.getAggregatedOrderedTeamStats?batch=1&input=${encodeURIComponent(JSON.stringify(p))}`);
  return d?.[0]?.result?.data?.json || [];
}
async function fetchMatches() {
  const p = {"0":{"json":{"seeOnlyCDL":true}},"1":{"json":{"seeOnlyCDL":true}}};
  const d = await proxyFetch(`https://www.breakingpoint.gg/api/trpc/matches.fetchLiveMatches,matches.fetchUpcomingMatches?batch=1&input=${encodeURIComponent(JSON.stringify(p))}`);
  return [...(d?.[0]?.result?.data?.json || []), ...(d?.[1]?.result?.data?.json || [])];
}
async function fetchRosters() {
  const d = await proxyFetch("https://www.breakingpoint.gg/_next/data/qc5mt7EbU7bkvD8K_eB00/en/cdl/teams-and-players.json");
  return d?.pageProps?.teams || [];
}
async function fetchStandings(eventId = null) {
  const filter = eventId ? `event_id=eq.${eventId}` : "event_id=is.null";
  return proxyFetch(`${SUPABASE_BASE}?${STANDINGS_FIELDS}&season_id=eq.2026&${filter}&order=rank.asc&apikey=${SUPABASE_KEY}`);
}

const s = (obj, key, def = 0) => obj?.[key] ?? def;
function utcToET(dt) { try { return new Date(dt).toLocaleString("en-US",{timeZone:"America/New_York",weekday:"short",month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}); } catch { return dt; } }
function timeUntil(dt) { const d=new Date(dt)-new Date(); if(d<0)return"LIVE"; const h=Math.floor(d/36e5),m=Math.floor((d%36e5)/6e4); return h>24?`${Math.floor(h/24)}d ${h%24}h`:h>0?`${h}h ${m}m`:`${m}m`; }
function kdColor(kd) { return kd>=1.05?"#52b788":kd>=1.0?"#a3be8c":kd>=0.95?"#ffd166":"#ff6b6b"; }

function buildAnalysis(players, teams, matches, rosters, seasonStandings, majorStandings) {
  const teamLookup={}, teamPlayers={}, playerTeam={}, playerStats={}, playerRoles={};
  rosters.forEach(t => {
    teamLookup[t.id] = {name:t.name, short:t.name_short, color:t.color_hex||"#888"};
    const active = (t.players||[]).filter(p=>!p.retired);
    teamPlayers[t.id] = active;
    active.forEach(p => { playerTeam[p.id]={teamName:t.name,teamShort:t.name_short,teamId:t.id}; playerRoles[p.id]=ROLE_MAP[p.position_id]||""; });
  });
  players.forEach(p => { const i=playerTeam[p.player_id]||{}; p.team_name=i.teamName||""; p.team_short=i.teamShort||""; p.team_id=i.teamId; p.role=playerRoles[p.player_id]||""; playerStats[p.player_id]=p; });
  const teamStats = {};
  teams.forEach(t => { const i=teamLookup[t.team_id]||{}; t.team_name=i.name||`Team ${t.team_id}`; t.team_short=i.short||""; t.team_color=i.color||"#888"; teamStats[t.team_id]=t; });
  const rosterStats = (tid) => (teamPlayers[tid]||[]).map(p=>playerStats[p.id]).filter(Boolean);

  const standingsLookup={}, majorStandingsLookup={};
  (seasonStandings||[]).forEach(st => { standingsLookup[st.team_id]=st; });
  (majorStandings||[]).forEach(st => { majorStandingsLookup[st.team_id]=st; });

  // Power rankings
  const power = Object.values(teamStats).map(ts => {
    const tid=ts.team_id, kd=s(ts,"kd"), hpW=s(ts,"hp_map_win_percentage"), sndW=s(ts,"snd_map_win_percentage"), ovlW=s(ts,"ovl_map_win_percentage");
    const avgWin=(hpW+sndW+ovlW)/3, rs=rosterStats(tid);
    const star=rs.reduce((b,p)=>s(p,"kd")>s(b,"kd")?p:b,rs[0]||{});
    const score=(kd*25)+(avgWin*0.3)+(s(ts,"hp_average_differential")*0.08)+(s(ts,"snd_average_differential")*1.5)+(s(ts,"ovl_average_differential")*0.5);
    const st=standingsLookup[tid]||{};
    return {tid,name:ts.team_name,short:ts.team_short,color:ts.team_color,score,kd,avgWin,hpW,sndW,ovlW,star:star?.player_tag||"",starKd:s(star,"kd"),matchWins:st.match_wins||0,matchLosses:st.match_losses||0,points:st.points||0,standingRank:st.rank||99};
  }).sort((a,b)=>b.score-a.score);
  const powerLookup={}; power.forEach(p=>{powerLookup[p.tid]=p;});

  const allPs = Object.values(playerStats).filter(p=>p.team_id);
  const topKd = [...allPs].sort((a,b)=>s(b,"kd")-s(a,"kd")).slice(0,5);
  const topHpK = [...allPs].sort((a,b)=>s(b,"hp_k_10m")-s(a,"hp_k_10m")).slice(0,5);
  const topSndKpr = [...allPs].sort((a,b)=>s(b,"snd_kpr")-s(a,"snd_kpr")).slice(0,5);

  const known=matches.filter(m=>m.team_1_id&&m.team_2_id), seen=new Set(), matchups=[];
  known.forEach(m=>{
    const key=`${Math.min(m.team_1_id,m.team_2_id)}-${Math.max(m.team_1_id,m.team_2_id)}-${m.events?.id}`;
    if(seen.has(key))return; seen.add(key);
    const t1=teamStats[m.team_1_id]||{},t2=teamStats[m.team_2_id]||{},p1=powerLookup[m.team_1_id]||{},p2=powerLookup[m.team_2_id]||{};
    const edge=Math.abs((p1.score||0)-(p2.score||0)), favored=(p1.score||0)>=(p2.score||0)?m.team1?.name_short:m.team2?.name_short;
    matchups.push({id:m.id,datetime:m.datetime,bestOf:m.best_of,t1:m.team1,t2:m.team2,event:m.events,round:m.round,t1Stats:t1,t2Stats:t2,t1Roster:rosterStats(m.team_1_id),t2Roster:rosterStats(m.team_2_id),p1,p2,edge,favored});
  });

  return {power,matchups,teamStats,playerStats:allPs,rosterStats,topKd,topHpK,topSndKpr,teamLookup,teamPlayers,powerLookup,standingsLookup,majorStandingsLookup,seasonStandings:seasonStandings||[],majorStandings:majorStandings||[]};
}

// ─── Shared Components ──────────────────────────────────────────────────────
function Stat({label,value,fmt="0.00",color}) {
  let d; if(typeof value==="number"){if(fmt==="pct")d=value.toFixed(1)+"%";else if(fmt==="int")d=Math.round(value);else if(fmt==="0.0")d=value.toFixed(1);else d=value.toFixed(2);}else d=value;
...
function KdBadge({kd,size="lg"}) { return <span className={size==="sm"?"font-mono font-bold text-sm":"font-mono font-bold text-lg"} style={{color:kdColor(kd)}}>{kd.toFixed(2)}</span>; }
function RoleBadge({role}) { if(!role)return null; const c={AR:"#53a8b6",SMG:"#e94560",Flex:"#ffd166"}; return <span className="text-xs px-1.5 py-0.5 rounded ml-1.5" style={{background:"rgba(255,255,255,0.08)",color:c[role]||"#888",fontSize:"10px"}}>{role}</span>; }

function H2HRow({label,v1,v2,fmt="0.00",higherBetter=true}) {
  const f=v=>fmt==="pct"?v.toFixed(1)+"%":fmt==="0.0"?v.toFixed(1):v.toFixed(2);
  const c1=higherBetter?(v1>v2?"#52b788":v1<v2?"#ff6b6b":"#ffd166"):(v1<v2?"#52b788":v1>v2?"#ff6b6b":"#ffd166");
  const c2=higherBetter?(v2>v1?"#52b788":v2<v1?"#ff6b6b":"#ffd166"):(v2<v1?"#52b788":v2>v1?"#ff6b6b":"#ffd166");
...
function PlayerRow({p}) {
  const maps=Math.round((s(p,"hp_game_count")+s(p,"snd_game_count")+s(p,"ovl_game_count"))/3);
...
function TeamRosterBlock({teamName,teamColor,roster}) {
  return <div><div className="flex items-center gap-2 mb-2 mt-1"><div className="w-1 h-4 rounded" style={{background:teamColor}}/><span className="text-xs font-bold text-white uppercase tracking-wider">{teamName}</span></div>{roster.map(p=><PlayerRow key={p.player_id} p={p}/>)}</div>;
}

// ─── Who's Hot ──────────────────────────────────────────────────────────────
function WhosHot({topKd,topHpK,topSndKpr}) {
  const [cat,setCat] = useState("kd");
  const list = cat==="kd"?topKd:cat==="hp"?topHpK:topSndKpr;
  const valFn = cat==="kd"?p=>s(p,"kd").toFixed(2):cat==="hp"?p=>s(p,"hp_k_10m").toFixed(1):p=>s(p,"snd_kpr").toFixed(2);
  const labelMap = {kd:"K/D",hp:"HP K/10m",snd:"SnD KPR"};
  return <div className="rounded-xl p-4 mb-6" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)"}}>
    <div className="flex items-center justify-between mb-3">
      ...
      <div className="flex gap-1">{["kd","hp","snd"].map(c=><button key={c} onClick={()=>setCat(c)} className="px-2 py-1 rounded text-xs font-semibold" style={{background:cat===c?"rgba(233,69,96,0.2)":"rgba(255,255,255,0.05)",color:cat===c?"#e94560":"#666"}}>{labelMap[c]}</button>)}</div>
    </div>
    {list.map((p,i)=><div key={p.player_id} className="flex items-center gap-3 py-1.5" style={{borderBottom:i<list.length-1?"1px solid rgba(255,255,255,0.03)":"none"}}>
...
// ─── Power Rankings ─────────────────────────────────────────────────────────
function PowerRankings({power}) {
  return <div className="space-y-2">{power.map((t,i)=>
    <div key={t.tid} className="p-3 rounded-lg" style={{background:i%2===0?"rgba(255,255,255,0.03)":"transparent"}}>
...
// ─── Match Card ─────────────────────────────────────────────────────────────
function MatchCard({mu,onTeamClick}) {
  const [expanded,setExpanded] = useState(false);
  const t1S=mu.t1?.name_short||"?", t2S=mu.t2?.name_short||"?", cd=timeUntil(mu.datetime);
  return <div className="rounded-xl overflow-hidden" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)"}}>
    <div className="p-4 cursor-pointer" onClick={()=>setExpanded(!expanded)}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs uppercase tracking-wider opacity-40">{mu.event?.name_short} · {(mu.round?.name_short||"").trim()} · Bo{mu.bestOf}</span>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2"><div className="w-2 h-8 rounded" style={{background:mu.t1Stats?.team_color||"#888"}}/><span className="font-bold text-xl text-white">{t1S}</span></div>
        ...
        <div className="flex items-center gap-2"><span className="font-bold text-xl text-white">{t2S}</span><div className="w-2 h-8 rounded" style={{background:mu.t2Stats?.team_color||"#888"}}/></div>
      </div>
    ...
    {expanded && <div>
      <div className="px-4 pb-3 pt-1" style={{borderTop:"1px solid rgba(255,255,255,0.05)"}}>
        ...
        <div className="grid grid-cols-3 items-center pb-1 mb-1" style={{borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
          <div className="text-right pr-3 text-xs font-bold cursor-pointer hover:underline" style={{color:mu.t1Stats?.team_color||"#888"}} onClick={(e)=>{e.stopPropagation();onTeamClick(mu.t1Stats?.team_id);}}>{t1S}</div>
          <div className="text-center text-xs opacity-30">vs</div>
          <div className="text-left pl-3 text-xs font-bold cursor-pointer hover:underline" style={{color:mu.t2Stats?.team_color||"#888"}} onClick={(e)=>{e.stopPropagation();onTeamClick(mu.t2Stats?.team_id);}}>{t2S}</div>
        </div>
      ...
      <div className="px-4 pb-4 pt-1" style={{borderTop:"1px solid rgba(255,255,255,0.05)"}}>
        ...
        <TeamRosterBlock teamName={t1S} teamColor={mu.t1Stats?.team_color} roster={mu.t1Roster||[]}/>
        <div className="mt-3"/>
        <TeamRosterBlock teamName={t2S} teamColor={mu.t2Stats?.team_color} roster={mu.t2Roster||[]}/>
      </div>
    </div>}
  </div>;
}
...
// ─── Team Page ──────────────────────────────────────────────────────────────
function TeamPage({tid,analysis,onBack}) {
  const ts = analysis.teamStats[tid]||{};
  const standing = analysis.standingsLookup[tid]||{};
  const major = analysis.majorStandingsLookup[tid]||{};
  const roster = analysis.rosterStats(tid);
  const teamMatches = analysis.matchups.filter(mu=>mu.t1Stats?.team_id===tid||mu.t2Stats?.team_id===tid);
  const mW=standing.match_wins||0, mL=standing.match_losses||0, gW=standing.game_wins||0, gL=standing.game_losses||0;
  const mmW=major.match_wins||0, mmL=major.match_losses||0, mgW=major.game_wins||0, mgL=major.game_losses||0;
  ...
  return <div>
    ...
    <div className="mb-5">
      ...
      <div className="rounded-lg overflow-hidden" style={{background:"rgba(255,255,255,0.02)"}}>
        ...
        {[["Hardpoint","hp"],["SnD","snd"],["Overload","ovl"]].map(([label,mode])=>{
          const w=s(ts,`${mode}_map_win_percentage`), kd=s(ts,`${mode}_kd`), diff=s(ts,`${mode}_average_differential`);
    ...
    <div className="mb-5"><div className="text-xs uppercase tracking-wider opacity-40 mb-3">Roster</div>{roster.map(p=><PlayerRow key={p.player_id} p={p}/>)}{roster.length===0&&<p className="text-sm opacity-40">No player stats available</p>}</div>
    <div><div className="text-xs uppercase tracking-wider opacity-40 mb-3">Upcoming schedule</div>
      ...
      {teamMatches.map(mu=>{
        const isT1=mu.t1Stats?.team_id===tid;
        const oppName=isT1?mu.t2?.name_short:mu.t1?.name_short;
        const oppColor=isT1?mu.t2Stats?.team_color:mu.t1Stats?.team_color;
        return <div key={mu.id} className="flex items-center justify-between p-3 rounded-lg mb-2" style={{background:"rgba(255,255,255,0.02)"}}>
          <div className="flex items-center gap-3"><div className="w-1 h-5 rounded" style={{background:oppColor||"#888"}}/><div><div className="text-sm font-semibold text-white">vs {oppName}</div><div style={{fontSize:"11px",color:"#555"}}>{mu.event?.name_short} · Bo{mu.bestOf}</div></div></div>
...
// ─── Team Cards Grid (ordered by Major 2) ───────────────────────────────────
function TeamsGrid({analysis,onTeamClick}) {
  // Order by major standings rank
  const ordered = [...analysis.majorStandings].sort((a,b)=>(a.rank||99)-(b.rank||99));
  return <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
    {ordered.map((major,i)=>{
      const tid=major.team_id;
      const ts=analysis.teamStats[tid]||{};
      const season=analysis.standingsLookup[tid]||{};
      const color=ts.team_color||"#888";
      const name=ts.team_short||major.team_name_short?.name_short||"?";
      const fullName=ts.team_name||major.team_name?.name||"?";
      const kd=s(ts,"kd");
      const hpDiff=s(ts,"hp_average_differential");
      const avgWin=((s(ts,"hp_map_win_percentage")+s(ts,"snd_map_win_percentage")+s(ts,"ovl_map_win_percentage"))/3);

      return <div key={tid} className="rounded-xl p-4 cursor-pointer hover:border-white/20 transition-all" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)"}} onClick={()=>onTeamClick(tid)}>
...
// ─── Player Search ──────────────────────────────────────────────────────────
function PlayerSearch({analysis}) {
  const [query,setQuery]=useState(""), [selected,setSelected]=useState(null);
  const results=useMemo(()=>{if(query.length<2)return[];const q=query.toLowerCase();return analysis.playerStats.filter(p=>p.player_tag?.toLowerCase().includes(q)||p.team_name?.toLowerCase().includes(q)).slice(0,12);},[query,analysis]);

  return <div className="space-y-3">
    <input type="text" value={query} onChange={e=>{setQuery(e.target.value);setSelected(null);}} placeholder="Search player or team..." className="w-full p-3 rounded-lg text-white placeholder-gray-500 outline-none" style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)"}}/>
    {results.length>0&&!selected&&<div className="space-y-1">{results.map(p=>
      <div key={p.player_id} className="flex items-center gap-3 p-2 rounded-lg cursor-pointer hover:bg-white/5" onClick={()=>setSelected(p)}>
    ...
    {selected&&<div className="space-y-3">
      <div className="flex items-center justify-between"><div><div className="flex items-center gap-2"><h3 className="text-xl font-bold text-white">{selected.player_tag}</h3><RoleBadge role={selected.role}/></div><span className="text-sm opacity-50">{selected.team_name}</span></div><button onClick={()=>setSelected(null)} className="text-xs opacity-50 hover:opacity-100">✕ clear</button></div>
...
// ─── App ────────────────────────────────────────────────────────────────────
const TABS=["Schedule","Rankings","Teams","Search"];

export default function App() {
  const [tab,setTab]=useState("Schedule"),[loading,setLoading]=useState(true),[error,setError]=useState(null),[analysis,setAnalysis]=useState(null);
  const [teamPageId,setTeamPageId]=useState(null);

  const openTeam=(tid)=>{setTeamPageId(tid);setTab("Teams");};
  const closeTeam=()=>setTeamPageId(null);

  useEffect(()=>{(async()=>{try{setLoading(true);setError(null);
    const[p,t,m,r,ss,ms]=await Promise.all([fetchPlayers(),fetchTeams(),fetchMatches(),fetchRosters(),fetchStandings(null),fetchStandings(102)]);
    setAnalysis(buildAnalysis(p,t,m,r,ss,ms));
  ...
  return <div className="min-h-screen" style={{background:"#0d0d1a",color:"#c8c8d0"}}>
    <div className="sticky top-0 z-50 backdrop-blur-xl" style={{background:"rgba(13,13,26,0.9)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
      <div className="max-w-4xl mx-auto px-4 py-3">
        ...
        <div className="flex gap-1 overflow-x-auto">{TABS.map(t=><button key={t} onClick={()=>{setTab(t);if(t!=="Teams")setTeamPageId(null);}} className="px-3 sm:px-4 py-1.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap" style={{background:tab===t?"#e94560":"transparent",color:tab===t?"#fff":"#666"}}>{t}</button>)}</div>
      </div>
    </div>
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      {tab==="Schedule"&&<div className="space-y-3">
        ...
        {analysis.matchups.map(mu=><MatchCard key={mu.id} mu={mu} onTeamClick={openTeam}/>)}
