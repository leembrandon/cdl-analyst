...
// ─── YOUTUBE FEED ────────────────────────────────────────────
// Fetches CDL-related videos from YouTube Data API via our proxy route.
// Falls back gracefully if the API key isn't configured yet.

var YT_CACHE_KEY = "barracks_yt_cache";
var YT_CACHE_TTL = 30 * 60 * 1000; // 30 minutes

async function fetchYouTubeVideos() {
  // Check cache first
  try {
    var cached = sessionStorage.getItem(YT_CACHE_KEY);
    if (cached) {
      var parsed = JSON.parse(cached);
      if (Date.now() - parsed.ts < YT_CACHE_TTL) return parsed.data;
    }
  } catch(e) {}
  ...
  try {
    ...
    var videos = (data.items || data || []).map(function(item) {
      var snippet = item.snippet || item;
      var id = (item.id && item.id.videoId) || item.videoId || item.id || "";
      return {
        id: id,
        title: snippet.title || "",
    ...
    // Cache it
    try { sessionStorage.setItem(YT_CACHE_KEY, JSON.stringify({ts: Date.now(), data: videos})); } catch(e) {}
    return videos;
  } catch(e) {
...
// ─── REDDIT FEED ─────────────────────────────────────────────
// Pulls hot posts from r/CoDCompetitive — no API key needed.

var REDDIT_CACHE_KEY = "barracks_reddit_cache";
var REDDIT_CACHE_TTL = 15 * 60 * 1000; // 15 minutes

async function fetchRedditPosts() {
  try {
    var cached = sessionStorage.getItem(REDDIT_CACHE_KEY);
    if (cached) {
      var parsed = JSON.parse(cached);
      if (Date.now() - parsed.ts < REDDIT_CACHE_TTL) return parsed.data;
    }
  } catch(e) {}
  ...
  return <div className="space-y-3">
    ...
    {selected && <div className="space-y-3">
      ...
      <div className="rounded-lg p-3" style={{background: "rgba(255,255,255,0.03)"}}>
        ...
        <PlayerStatCard player={selected} />
      </div>
    </div>}
  </div>;
}
...
function PlayerStatCard(props) {
  ...
  var handleShare = function() {
    if (sharing) return;
    setSharing(true);
    var shareUrl = window.location.origin + window.location.pathname + "?player=" + encodeURIComponent(p.gamertag);
    import("./shareRenderer.js").then(function(mod) {
      return mod.shareStatCard(p, shareUrl);
    }).catch(function(e) { console.error("Share error:", e); }).finally(function() { setSharing(false); });
  };

  var handleCopyLink = function() {
    var url = window.location.origin + window.location.pathname + "?player=" + encodeURIComponent(p.gamertag);
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url).then(function() { setCopied(true); setTimeout(function() { setCopied(false); }, 2000); }).catch(function() { prompt("Copy this link:", url); });
    } else {
      prompt("Copy this link:", url);
    }
  };
  ...
  return <div className="flex gap-2 mt-2">
    <button onClick={handleShare} disabled={sharing} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all" style={{
      background: sharing ? "rgba(233,69,96,0.2)" : "rgba(233,69,96,0.1)",
      color: sharing ? "#888" : "#e94560",
      border: "1px solid rgba(233,69,96,0.2)"
    }}>
      {sharing ? <div className="w-3 h-3 border-2 border-t-transparent rounded-full animate-spin" style={{borderColor: "#e94560", borderTopColor: "transparent"}} /> : <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>}
      <span>{sharing ? "..." : "Share card"}</span>
    </button>
    <button onClick={handleCopyLink} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all" style={{
      background: copied ? "rgba(82,183,136,0.15)" : "rgba(255,255,255,0.06)",
      color: copied ? "#52b788" : "#888",
      border: copied ? "1px solid rgba(82,183,136,0.3)" : "1px solid rgba(255,255,255,0.08)"
    }}>
...
function FeedTab() {
  var [ytVideos, setYtVideos] = useState([]);
  var [redditPosts, setRedditPosts] = useState([]);
  var [loading, setLoading] = useState(true);
  var [feedType, setFeedType] = useState("all"); // all, youtube, reddit
  var [ytError, setYtError] = useState(false);
  var [redditError, setRedditError] = useState(false);
  ...
  useEffect(function() {
    (async function() {
      ...
      if (results[0].status === "fulfilled") {
        setYtVideos(results[0].value || []);
        setYtError((results[0].value || []).length === 0);
      } else { setYtError(true); }
      if (results[1].status === "fulfilled") {
        setRedditPosts(results[1].value || []);
        setRedditError((results[1].value || []).length === 0);
      } else { setRedditError(true); }
      setLoading(false);
    })();
  }, []);

  if (loading) return <div className="py-12 text-center"><div className="w-8 h-8 border-2 border-t-transparent rounded-full animate-spin mx-auto" style={{borderColor: "#e94560", borderTopColor: "transparent"}} /><p className="text-sm mt-3" style={{color: "#555"}}>Loading feed...</p></div>;

  var hasYt = ytVideos.length > 0;
  var hasReddit = redditPosts.length > 0;

  if (!hasYt && !hasReddit) return <div className="text-center py-12">
    <div className="text-3xl mb-3" style={{opacity: 0.3}}>📡</div>
    <p className="text-sm" style={{color: "#555"}}>No feed content available right now.</p>
    <p className="text-xs mt-2" style={{color: "#444"}}>YouTube API and Reddit r/CoDCompetitive will appear here once connected.</p>
    <div className="mt-4 rounded-xl p-4" style={{background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)"}}>
      <p className="text-xs mb-2" style={{color: "#888"}}>To enable YouTube:</p>
      <p className="text-xs" style={{color: "#555"}}>Create <code style={{color: "#e94560"}}>/api/youtube.js</code> route that proxies YouTube Data API v3 search results for CDL content.</p>
    </div>
  </div>;

  // Build mixed feed sorted by date
  var allItems = [];
  if (feedType === "all" || feedType === "youtube") {
    ytVideos.forEach(function(v) {
      allItems.push({type: "youtube", data: v, date: new Date(v.publishedAt || 0)});
    });
  }
  if (feedType === "all" || feedType === "reddit") {
    redditPosts.forEach(function(p) {
      allItems.push({type: "reddit", data: p, date: p.created || new Date(0)});
    });
  }
  ...
  return <div>
    <div className="mb-4">
      <h2 className="text-lg font-bold text-white mb-1">CDL Feed</h2>
      <p className="text-xs" style={{color: "#555"}}>Latest from YouTube & r/CoDCompetitive</p>
    </div>

    {/* Feed type filter */}
    <div className="flex gap-1.5 mb-4">
      {[{key: "all", label: "All"}, {key: "youtube", label: "YouTube", disabled: !hasYt}, {key: "reddit", label: "Reddit", disabled: !hasReddit}].map(function(f) {
        if (f.disabled) return null;
        var isActive = feedType === f.key;
        return <button key={f.key} onClick={function() { setFeedType(f.key); }} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all" style={{
          background: isActive ? "rgba(233,69,96,0.2)" : "rgba(255,255,255,0.04)",
          color: isActive ? "#e94560" : "#666",
          border: isActive ? "1px solid rgba(233,69,96,0.3)" : "1px solid rgba(255,255,255,0.06)"
        }}>{f.label}</button>;
    ...
    <div className="space-y-3">
      {allItems.map(function(item, i) {
        if (item.type === "youtube") {
          var v = item.data;
          return <a key={"yt-" + v.id} href={"https://www.youtube.com/watch?v=" + v.id} target="_blank" rel="noopener noreferrer" className="block rounded-xl overflow-hidden transition-all hover:border-white/20" style={{background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", textDecoration: "none"}}>
            {v.thumbnail && <div className="relative">
              <img src={v.thumbnail} alt="" style={{width: "100%", height: "auto", display: "block", aspectRatio: "16/9", objectFit: "cover"}} />
              <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-md" style={{background: "rgba(255,0,0,0.85)", backdropFilter: "blur(4px)"}}>
                ...
                <span style={{fontSize: "9px", fontWeight: 700, color: "#fff"}}>YouTube</span>
              </div>
              <div className="absolute bottom-0 left-0 right-0" style={{background: "linear-gradient(transparent, rgba(13,13,26,0.95))", height: "40px"}} />
            </div>}
            <div className="p-3">
              <div className="text-sm font-bold text-white mb-1" style={{lineHeight: "1.3"}}>{v.title}</div>
              <div className="flex items-center gap-2">
                <span className="text-xs" style={{color: "#888"}}>{v.channel}</span>
                <span className="text-xs" style={{color: "#444"}}>·</span>
                <span className="text-xs" style={{color: "#555"}}>{timeAgo(v.publishedAt)}</span>
              </div>
            </div>
          </a>;
        }
        ...
        if (item.type === "reddit") {
          var p = item.data;
          return <a key={"rd-" + p.id} href={p.url} target="_blank" rel="noopener noreferrer" className="block rounded-xl p-4 transition-all hover:border-white/20" style={{background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", textDecoration: "none"}}>
            <div className="flex items-start gap-3">
              {/* Upvote indicator */}
              <div className="flex flex-col items-center flex-shrink-0 pt-0.5" style={{minWidth: "32px"}}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="#ff4500" style={{marginBottom: "2px"}}><path d="M12 4l-8 8h5v8h6v-8h5z"/></svg>
                <span className="text-xs font-bold" style={{color: p.score > 50 ? "#ff4500" : "#888"}}>{p.score > 999 ? (p.score / 1000).toFixed(1) + "k" : p.score}</span>
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex items-center gap-1 px-1.5 py-0.5 rounded" style={{background: "rgba(255,69,0,0.12)"}}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="#ff4500"><circle cx="12" cy="12" r="10"/><circle cx="8" cy="10" r="2" fill="#fff"/><circle cx="16" cy="10" r="2" fill="#fff"/><path d="M8 15c0 0 2 3 4 3s4-3 4-3" stroke="#fff" strokeWidth="1.5" fill="none"/></svg>
                    <span style={{fontSize: "9px", fontWeight: 700, color: "#ff4500"}}>Reddit</span>
                  </div>
                  {p.flair && <span className="text-xs px-1.5 py-0.5 rounded" style={{background: "rgba(255,255,255,0.06)", color: "#888", fontSize: "9px"}}>{p.flair}</span>}
                </div>
                <div className="text-sm font-bold text-white mb-1.5" style={{lineHeight: "1.3"}}>{p.title}</div>
                <div className="flex items-center gap-2">
                  <span className="text-xs" style={{color: "#888"}}>u/{p.author}</span>
                  <span className="text-xs" style={{color: "#444"}}>·</span>
                  <span className="text-xs" style={{color: "#555"}}>{timeAgo(p.created)}</span>
                  <span className="text-xs" style={{color: "#444"}}>·</span>
                  <span className="text-xs" style={{color: "#555"}}>{p.numComments} comments</span>
                </div>
              </div>
            </div>
          </a>;
    ...
    {allItems.length > 0 && <div className="text-center py-4 mt-2">
      <p className="text-xs" style={{color: "#444"}}>Content from YouTube & r/CoDCompetitive · Auto-refreshes every 15-30 min</p>
    </div>}
  </div>;
}
